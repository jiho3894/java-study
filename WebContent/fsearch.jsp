...
<%
try{
	String b_title, b_name;  /*  String 형 변수 선언  */  
	int b_id;  /* 정수형 변수 선언 */ 
...
/* 목록페이지에서 페이징 처리 하는 코드  */ 
   ...
   ResultSet rs = stmt.executeQuery("select b_id, b_title, b_name from mboard order by b_id desc"); 
   if(!rs.next()){
	   pagesize = 0; 
   } else {
	   rs.absolute(abpage); 
